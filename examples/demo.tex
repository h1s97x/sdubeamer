...
%   （模板 sdubeamer.sty 位于仓库根目录 src/ 下，需确保可被 TEXINPUTS 找到）
%
% 亦可用 latexmk：
...
% 页脚学校名默认为「山东大学」，可通过 school 选项自定义：
% \usepackage[school=山东大学 数学学院]{sdubeamer}

% 封面英文校名默认为 Shandong University，可通过 enname 选项自定义：
% \usepackage[enname=School of Mathematics, Shandong University]{sdubeamer}

% 校徽文件路径默认为 assets/sdu-logo.jpg，可通过 logo 选项自定义：
% \usepackage[logo=assets/my-logo.png]{sdubeamer}

% 答辩类型默认为 master，可选：undergrad / master / doctor
...
% 页眉默认显示当前章节名，页脚左侧显示学校名、右侧显示页码。
% 如需在正文中临时覆盖页脚学校名，可使用 \sdufooter{}：
...
\begin{frame}{列表与超链接}
  \begin{enumerate}
    \item 一级编号使用 SDU 主色
    \item 二级编号使用辅助色\begin{enumerate}\item 缩进子项\item 缩进子项\end{enumerate}
    \item 三级使用强调色
  \end{enumerate}

  \vspace{0.5cm}
  超链接配色已定制，例如访问 \href{https://www.sdu.edu.cn}{山东大学官网}。
\end{frame}
